...
class UserFriendshipsControllerTest < ActionController::TestCase
  ...
  context "#index" do
    ...
    context "when logged in" do
      setup do
        @friendship1 = create(:pending_user_friendship, user: users(:mezbah), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
        @friendship2 = create(:accepted_user_friendship, user: users(:mezbah), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
        @friendship3 = create(:requested_user_friendship, user: users(:mezbah), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
        @friendship4 = user_friendships(:blocked_by_mezbah)
      ...
      should "display pending a information on a pending friendship" do
        assert_select "#user_friendship_#{@friendship1.id}" do
          assert_select "em", "Friendship is pending."
        end
      end

      should "display datea information on a accepted friendship" do
  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "display the friend's name" do
        get :new, friend_id: users(:mezba)
        assert_match /#{users(:mezba).full_name}/, response.body
      end
      ...
      should "assign a new user friendship to the correct friend" do
        get :new, friend_id: users(:mezba)
        assert_equal users(:mezba), assigns(:user_friendship).friend
        end

      should "assign a new user friendship to the currently logged in user" do
        get :new, friend_id: users(:mezba)
        assert_equal users(:mezbah), assigns(:user_friendship).user
      end
      ...
      should "ask if you really want to friend the user" do
        get :new, friend_id: users(:mezba)
        assert_match /Do you really want to friend #{users(:mezba).full_name}?/, response.body
      end
    end
  end
  ...
  context "#create" do
    ...
    context "when logged in" do
      ...
      context "with a valid friend_id" do
        setup do
          post :create, user_friendship: { friend_id: users(:mez) }
        end

        should "assign a friend object" do
          assert assigns(:friend)
          assert_equal users(:mez), assigns(:friend)
        end

        should "assign a user_friendship object" do
          assert assigns(:user_friendship)
          assert_equal users(:mezbah), assigns(:user_friendship).user
          assert_equal users(:mez), assigns(:user_friendship).friend
        end
        ...
        should "redirect to the profile page of the friend" do
          assert_response :redirect
          assert_redirected_to profile_path(users(:mez))
        end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:mezbah), friend: @friend)
        create(:pending_user_friendship, friend: users(:mezbah), user: @friend)
  ...
  context "#destroy" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:mezbah))
        create(:accepted_user_friendship, friend: users(:mezbah), user: @friend)
